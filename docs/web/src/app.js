...
function setupThemeToggle() {
  ...
  const key = document.documentElement.dataset.themeKey || "forge-lua-docs-theme";
...
function markActiveNav() {
  ...
  for (const link of document.querySelectorAll(".nav-section a")) {
...
function renderResults(results) {
  ...
  const html = results
      .slice(0, 8)
      .map((item) => `<a href="${item.url}">${item.title}<span>${item.section}</span></a>`)
      .join("");
